...
import { TypeExam } from 'src/util/constants';
...
export class SectionService {
  ...
  async deleteSection(id: any) {
    ...
    await queryRunner.startTransaction()
    try {
      const section = await queryRunner.manager.withRepository(this.sectionRp).findOneBy({ id: id })
      if (section.type == TypeExam) {
        await queryRunner.manager.withRepository(this.quizRp).delete({ id: section.idReference })
      }
      await queryRunner.manager.withRepository(this.sectionRp).delete({ id: id })
      await queryRunner.commitTransaction()
